	...
	const dataneeded={
		...
		useEffect(() => {
			...
			const getUserWebsite = () =>{
				try {
					if(!Stop) {
						fetch(process.env.REACT_APP_Local_Server_IP+":5000/api/user", {
							method: "GET",
							credentials: "include",
							headers: {
			...
			const getUsergoogle = () => {
				try{
					if(!Stop ) {
						fetch(process.env.REACT_APP_Local_Server_IP+":5000/auth/login/success", {
							method: "GET",
							credentials: "include",
					  headers: {
						  Accept: "application/json",
			...
			const get_allmangas = () => {
				fetch(process.env.REACT_APP_Local_Server_IP + ":5001/m", {
									method: "GET",
									credentials: "include",
									headers: {
										Accept: "application/json",
			...
			const get_allmangatags = () => {
				fetch(process.env.REACT_APP_Local_Server_IP + ":5001/tags/", {
									method: "GET",
									credentials: "include",
									headers: {
										Accept: "application/json",
			...
			const getAdminChecking = () =>{
				try {
					if(!Stop) {
						fetch(process.env.REACT_APP_Local_Server_IP+":5000/api/CheckForAdminCookie", {
							method: "GET",
							credentials: "include",
							headers: {
								Accept: "application/json",
