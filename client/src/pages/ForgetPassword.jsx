...
function ForgetPassword() {
	...
	async function SumbitEmail(event) {
		...
		const response = await fetch(process.env.REACT_APP_Local_Server_IP+':5000/api/checkemail', {
			method: 'POST',
			headers: {
		...
		if(data.status==='ok'){
			...
			await fetch(process.env.REACT_APP_Local_Server_IP+':5000/api/send_mail', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
