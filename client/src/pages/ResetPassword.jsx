...
function ResetPassword() {
	...
	async function SumbitPasswords(event) {
		...
		if(Password_1 === Password_2 && Password_1.length > 5 && Password_2.length > 5 && Password_2.length > 5){
			const response = await fetch(process.env.REACT_APP_Local_Server_IP +':5000/api/Update_Password', {
				method: 'POST',
				headers: {
	...
	useEffect(async () => {
		const response = await fetch(process.env.REACT_APP_Local_Server_IP +':5000/api/Did_You_Order_Pass', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
				},
