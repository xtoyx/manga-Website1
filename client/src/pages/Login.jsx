...
function App() {
	...
	const google = () => {
		window.open(process.env.REACT_APP_Local_Server_IP+":5000/auth/google", "_self");
	};
	
	const twitter=() => {
		window.open(process.env.REACT_APP_Local_Server_IP+":5000/auth/twitter",'_self')
	}
	const github = () => {
		window.open(process.env.REACT_APP_Local_Server_IP +":5000/auth/github", "_self");
	};
	
	const facebook = () => {
		window.open(process.env.REACT_APP_Local_Server_IP+":5000/auth/facebook", "_self");
	};

	async function loginUser(event) {
		event.preventDefault()
		if(!Checkedbox){
			const response = await fetch(process.env.REACT_APP_Local_Server_IP +':5000/api/login', {
				method: 'POST',
				headers: {
			...
			const response = await fetch(process.env.REACT_APP_Local_Server_IP+':5000/api/loginAndRemberMe', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
				},
					...
					<div className="row">
						...
						<div className="col-lg-6">
							<div className="login__register">
									...
									<a href={process.env.REACT_APP_Local_Server_IP + 'register'} className="primary-btn">Register Now</a>
							</div>
						</div>
					</div>
